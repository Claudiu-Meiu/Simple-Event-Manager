...
// Enqueue CSS file
function amicii_event_manager_enqueue_styles() {
    wp_enqueue_style('simple-event-manager-style', plugin_dir_url(__FILE__) . 'simple-event-manager.css');
}
add_action('wp_enqueue_scripts', 'amicii_event_manager_enqueue_styles');

// Register activation hook
register_activation_hook(__FILE__, 'amicii_event_manager_create_tables');

// Create tables in the database
function amicii_event_manager_create_tables() {
    global $wpdb;
...
// Update the event form shortcode function
function event_form_shortcode() {
    ...
    <div class="event-form">
        <form id="event-form" method="post">
            ...
            <p>
                ...
                <textarea id="event-description" name="event_description" required style="height: 15rem !important;"></textarea>
            </p>
            <p>
    ...
    if ($all_events) {
        ...
        foreach ($all_events as $event) {
            ...
            echo '<p><strong>Closing Date: </strong> ' . esc_html($event['end_date']) . '</p>';
            ...
            if ($current_user_id == $event['user_id']) {
                // If yes, display the delete and edit buttons
                echo '<form method="post" class="delete-form">';
                echo '<input type="hidden" name="event_id" value="' . esc_attr($event['id']) . '">';
                echo '<input type="submit" name="delete_event" value="Remove" class="delete-button" style="background-color: #ff5733; padding: 5px 10px;">';
                echo '</form>';

                // Display the edit form
                echo '<form method="post" style="margin-top: 3rem !important;">';
                echo '<input type="hidden" name="event_id" value="' . esc_attr($event['id']) . '">';
                echo '<p>';
                echo '<label for="event-title" style="display: block !important; margin-bottom: 5px !important;">Title:</label>';
                echo '<input style="border-radius: 5px !important; padding: 10px 24px !important; background-color: white !important; color: black; width: 100%;" type="text" id="event-title" name="event_title" value="' . esc_attr($event['event_title']) . '" required>';
                echo '</p>';
                echo '<p>';
                echo '<label for="event-description" style="display: block !important; margin-bottom: 5px !important;">Event description:</label>';
                echo '<textarea style="border-radius: 5px !important; background-color: white !important; color: black; height: 15rem !important; width: 100%;" id="event-description" name="event_description" required>' . esc_html($event['event_description']) . '</textarea>';
                echo '</p>';
                echo '<p>';
                echo '<label for="start-date" style="display: block !important; margin-bottom: 5px !important;">Start date:</label>';
                echo '<input style="border-radius: 5px !important; background-color: white !important; color: black; width: 100%;" type="date" id="start-date" name="start_date" value="' . esc_attr($event['start_date']) . '" required>';
                echo '</p>';
                echo '<p>';
                echo '<label for="end-date" style="display: block !important; margin-bottom: 5px !important;">Closing Date:</label>';
                echo '<input  style="border-radius: 5px !important; background-color: white !important; color: black; width: 100%;" type="date" id="end-date" name="end_date" value="' . esc_attr($event['end_date']) . '" required>';
                echo '</p>';
                echo '<p>';
                echo '<label for="event-location" style="display: block !important; margin-bottom: 5px !important;">Event location:</label>';
                echo '<textarea style="border-radius: 5px !important; background-color: white !important; color: black; width: 100%;" id="event-location" name="event_location" required>' . esc_html($event['event_location']) . '</textarea>';
                echo '</p>';
                echo '<p>';
                echo '<input  class="update__button" type="submit" name="update_event" value="Update Event" style="border-radius: 5px !important; padding: 10px 24px !important; background-color: #069059 !important;">';
                echo '</p>';
                echo '</form>';
            }
            ...
            if (!$has_clicked) {
                ...
                echo '<button type="submit" class="unregister-click-button" style="padding: 10px 10px; background: #ff5733;">I no longer participate</button>';
                echo '</form>';
            }
...
function update_event($event_id) {
    ...
    $event_description = isset($_POST['event_description']) ? sanitize_textarea_field($_POST['event_description']) : '';
    $start_date = isset($_POST['start_date']) ? sanitize_text_field($_POST['start_date']) : '';
    $end_date = isset($_POST['end_date']) ? sanitize_text_field($_POST['end_date']) : '';
    $event_location = isset($_POST['event_location']) ? sanitize_textarea_field($_POST['event_location']) : '';

    if (!$event_title || !$event_description || !$start_date || !$end_date || !$event_location) {
        return new WP_Error('missing_data', 'Toate câmpurile sunt obligatorii.');
    }
    ...
    if ($updated === false) {
        return new WP_Error('db_update_error', 'Error while updating the event.');
    }
